...
parser.add_option('--unitsPerJob',help='number of jobs to run')
parser.add_option('--nrEvents',help='number of events to gen per job')
parser.add_option('--publish',help="publish or not")
parser.add_option('--crabDryRun',help="crab dry run",default=False)
options,args = parser.parse_args()
if not options.dataset or not options.datasetTag or not options.outDir or not options.config or not options.unitsPerJob or not options.nrEvents or not options.publish:
    parser.error("dataset, datasetTag, config, nrEvents, unitsPerJob, publish and outDir are manditory")
...
dataset = options.dataset.split("/")[1]
crabProjDir='crab_projects'

datasetFormat="GEN-SIM"

outputFile=dataset+"_"+pubName+"_"+datasetFormat+".root"
...
workingDir="MC_"+options.outDir.replace("/","-")+"_"+dataset+"_"+pubName+"_"+currTime
...
crabSubmitCmd = "crab submit --wait -c crab_base.py  General.requestName="+workingDirTmp+ \
    " General.transferOutputs="+str(options.copyData)+ \
    " Data.inputDataset="+options.dataset+ \
    " Data.inputDBS="+options.dbsUrl+ \
    " Data.unitsPerJob="+str(options.unitsPerJob)+ \
    " Data.totalUnits="+str(options.nrEvents)+ \
