...
heep::ElectronTkIsolation::ElectronTkIsolation (double extRadius,
...
heep::ElectronTkIsolation::~ElectronTkIsolation ()
{}

std::pair<int,double> heep::ElectronTkIsolation::getIso(const reco::GsfElectron* electron) const {
  return getIso(&(*(electron->gsfTrack())));
}

// unified acces to isolations
std::pair<int,double> heep::ElectronTkIsolation::getIso(const reco::Track* tmpTrack) const  
{
  int counter  =0 ;
  double ptSum =0.;
...
int heep::ElectronTkIsolation::getNumberTracks (const reco::GsfElectron* electron) const
{  
  //counter for the tracks in the isolation cone
  return getIso(electron).first ;
}

double heep::ElectronTkIsolation::getPtTracks (const reco::GsfElectron* electron) const
{
  return getIso(electron).second ;
}


bool heep::ElectronTkIsolation::passAlgo(const reco::TrackBase& trk)const
{
  int algo = trk.algo();
...
void heep::ElectronTkIsolation::setAlgosToReject(std::vector<int> algos)
{
