...
private:
  ...
  std::unique_ptr<HLTConfigProvider> hltConfig_;
  std::unique_ptr<HLTConfigProvider> hltConfigP5_;
...
TrigResultTreeMakerV2::TrigResultTreeMakerV2(const edm::ParameterSet& iPara):tree_(nullptr)
{
  ...
  hltConfig_ = std::make_unique<HLTConfigProvider>();//iPara,consumesCollector(),*this);
  hltConfigP5_ = std::make_unique<HLTConfigProvider>();//iPara,consumesCollector(),*this);
...
void TrigResultTreeMakerV2::beginRun(const edm::Run& run,const edm::EventSetup& setup)
{
  ...
  if(pathNames_.empty()){ //initialising hlt paths names
    ...
    pathNamesP5_ = hltConfigP5_->triggerNames();
...
void TrigResultTreeMakerV2::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  psColumn_= -1;// hltConfigP5_->prescaleSet(iEvent,iSetup);
  hltPhysicsPS_ =-1;// = hltConfigP5_->prescaleValues(iEvent,iSetup,refTrigger_).second;
